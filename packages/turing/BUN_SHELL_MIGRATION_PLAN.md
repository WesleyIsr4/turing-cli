...
Replace runtime Bun shell template-tag usage in `packages/turing/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/turing/src` is removed except:
