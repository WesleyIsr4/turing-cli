...
- Author package entrypoint is `@turing-ai/plugin/tui`.
...
```json
{
  "$schema": "https://turing.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/turing-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@turing-ai/plugin/tui`.
- `@turing-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@turing-ai/plugin/tui"
...
- If `engines.turing` is absent, no check is performed (backward compatible).
...
- `turing plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `turing plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.turing` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.turing`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.turing`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.turing` dir or `TURING_CONFIG_DIR`, OpenCode installs `@turing-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@turing-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.turing/themes` area near the plugin config source.
...
- `--pure` / `TURING_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.turing/plugins/tui-smoke.tsx`
- Local vim plugin: `.turing/plugins/tui-vim.tsx`
- Local smoke config: `.turing/tui.json`
- Local smoke theme: `.turing/plugins/smoke-theme.json`
