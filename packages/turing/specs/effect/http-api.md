...
Practical notes for an eventual migration of `packages/turing` server routes from the current Hono handlers to Effect `HttpApi`, either as a full replacement or as a parallel surface.
...
The bridge is gated behind `TURING_EXPERIMENTAL_HTTPAPI` (or `TURING_EXPERIMENTAL`). When the flag is off (default), all requests go through the original Hono handlers unchanged.

```ts
// in instance/index.ts
if (Flag.TURING_EXPERIMENTAL_HTTPAPI) {
...
6. Mount the new surface in parallel behind the `TURING_EXPERIMENTAL_HTTPAPI` bridge.
...
- routes keep their normal instance paths and are gated by the `TURING_EXPERIMENTAL_HTTPAPI` flag
...
- the bridged `HttpApi` layer resolves instance context via an `HttpRouter.middleware` that reads `x-turing-directory` headers and `directory` query params
...
- the `TURING_EXPERIMENTAL_HTTPAPI` flag gates the bridge at the Hono router level — default off, no behavior change unless opted in.
...
- [x] gate behind `TURING_EXPERIMENTAL_HTTPAPI` flag
