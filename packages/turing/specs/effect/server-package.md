...
Practical reference for extracting a future `packages/server` from the current `packages/turing` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all turing services, Effect-first source of truth
- `packages/server` - turing server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full turing server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/turing`.
...
- those slices are mounted into the Hono server behind `TURING_EXPERIMENTAL_HTTPAPI`
...
Use `trinca/opentunnel` as the structural reference for `packages/server`.
...
The intended direction here is the same, but the current `turing` package split is earlier in the migration.
...
- the current server host still lives in `packages/turing`
...
- if `packages/server` imports services from `packages/turing` while `packages/turing` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/turing` as the temporary Hono host
...
- `packages/server` must not import from `packages/turing`

Allowed in phase 1:

- `packages/turing` imports `packages/server`
...
- `packages/turing` shrinks or disappears as package responsibilities are fully split
...
- `packages/turing` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/turing`
...
Keep in `packages/turing` for now:
...
- host mounting from `packages/turing`
...
- no imports from `packages/turing` inside `packages/server`
...
- `packages/server` must still not import from `packages/turing`
...
### PR 4. Mount `packages/server` question from `packages/turing`

Scope:

- replace local experimental question route wiring in `packages/turing`
...
- mount it in parallel from `packages/turing`
...
- move adapter selection and server startup out of `packages/turing`

Rules:

- do not start this while `packages/server` still depends on `packages/turing`
...
- host logic no longer lives in `packages/turing`
...
3. the route is mounted from the current host in `packages/turing`
...
- `packages/server` imports services or runtime from `packages/turing`
- `packages/turing` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/turing` imports
...
- [ ] mount `question` from `packages/turing`
...
2. keep `packages/turing` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/turing`, stop and restructure the boundary first.
