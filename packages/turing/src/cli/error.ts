...
export function FormatError(input: unknown) {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    return `MCP server "${(input as ErrorLike).data?.name}" failed. Note, turing does not support MCP authentication yet.`
  }
  ...
  if (NamedError.hasName(input, "ProviderModelNotFoundError")) {
    ...
    return [
      ...
      `Try: \`turing models\` to list available models`,
      `Or check your config (turing.json) provider/model names`,
    ].join("\n")
  }
