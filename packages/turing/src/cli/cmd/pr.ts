...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run turing",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for turing session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found turing session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["turing", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting turing...")
        UI.println()

        const turingArgs = sessionId ? ["-s", sessionId] : []
        const turingProcess = Process.spawn(["turing", ...turingArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await turingProcess.exited
        if (code !== 0) throw new Error(`turing exited with code ${code}`)
      },
    })
  },
})
