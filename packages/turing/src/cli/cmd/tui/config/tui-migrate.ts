...
const TUI_SCHEMA_URL = "https://turing.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from turing.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const turing = await turingFiles(input)
  for (const file of turing) {
...
async function turingFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "turing"),
    ...(await Filesystem.findUp(["turing.json", "turing.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "turing"))
  }
  if (Flag.TURING_CONFIG) files.push(Flag.TURING_CONFIG)
