...
import turing from "./theme/turing.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@turing-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "turing")
        draft.active = typeof active === "string" ? active : "turing"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.turing, store.mode)
    })
