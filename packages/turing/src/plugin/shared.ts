...
export async function checkPluginCompatibility(target: string, turingVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(turingVersion) || semver.major(turingVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.turing
  if (typeof range !== "string") return
  if (!semver.satisfies(turingVersion, range)) {
    throw new Error(`Plugin requires turing ${range} but running ${turingVersion}`)
  }
}
