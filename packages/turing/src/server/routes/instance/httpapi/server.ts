...
const Headers = Schema.Struct({
  ...
  "x-turing-directory": Schema.optional(Schema.String),
})
...
class Authorization extends HttpApiMiddleware.Service<Authorization>()("@turing/ExperimentalHttpApiAuthorization", {
  error: Unauthorized,
  security: {
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.TURING_SERVER_PASSWORD) return yield* effect

        const user = Flag.TURING_SERVER_USERNAME ?? "turing"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.TURING_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-turing-directory"] || process.cwd()
