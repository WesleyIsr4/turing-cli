...
import { Slug } from "@turing-ai/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    TURING_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    TURING_WORKSPACE_ID: config.id,
    TURING_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.TURING_EXPERIMENTAL_WORKSPACES) return
