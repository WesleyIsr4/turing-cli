...
const MANAGED_PLIST_DOMAIN = "ai.turing.managed"

// Keys injected by macOS/MDM into the managed plist that are not Turing config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/turing"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "turing")
    default:
      return "/etc/turing"
  }
}
...
export function managedConfigDir() {
  return process.env.TURING_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
