...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in turing config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for turing serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://turing.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://turing.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@turing/Config") {}

function globalConfigFile() {
  const candidates = ["turing.jsonc", "turing.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://turing.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "turing.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "turing.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://turing.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "TURING_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/turing` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/turing`))
            if (!response.ok) {
              throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
            }
            const wellknown = (yield* Effect.promise(() => response.json())) as { config?: Record<string, unknown> }
            const remoteConfig = wellknown.config ?? {}
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://turing.ai/config.json"
            const source = `${url}/.well-known/turing`
        ...
        if (Flag.TURING_CONFIG) {
          yield* merge(Flag.TURING_CONFIG, yield* loadFile(Flag.TURING_CONFIG))
          log.debug("loaded custom config", { path: Flag.TURING_CONFIG })
        }

        if (!Flag.TURING_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("turing", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.TURING_CONFIG_DIR) {
          log.debug("loading config from TURING_CONFIG_DIR", { path: Flag.TURING_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".turing") || dir === Flag.TURING_CONFIG_DIR) {
            for (const file of ["turing.json", "turing.jsonc"]) {
          ...
          // Auto-discovered plugins under `.turing/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.TURING_CONFIG_CONTENT) {
          const source = "TURING_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.TURING_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from TURING_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["TURING_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("TURING_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.TURING_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.TURING_PERMISSION))
        }
        ...
        if (Flag.TURING_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.TURING_DISABLE_PRUNE) {
