...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for turing.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal turing sessions
...
turing acp --cwd /path/to/project
```
...
```bash
TURING_ENABLE_QUESTION_TOOL=1 turing acp
```
...
- **Terminal Integration**: Full terminal support via turing's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | turing acp
```
...
ACP sessions map cleanly to turing's internal session model:
