...
import type { AssistantMessage, Event, TuringClient, SessionMessageResponse, ToolPart } from "@turing-ai/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: TuringClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `turing auth login` in the terminal",
      name: "Login with turing",
      id: "turing-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Turing stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  return { providerID: ProviderID.claudeCode, modelID: ModelID.make("default") }
}
