...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under turing.", () => {
    expect(paramToAttributeKey("name")).toBe("turing.name")
    expect(paramToAttributeKey("slug")).toBe("turing.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["turing.sessionID"]).toBeUndefined()
    expect(attrs["turing.messageID"]).toBeUndefined()
    expect(attrs["turing.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under turing. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["turing.name"]).toBe("exa")
