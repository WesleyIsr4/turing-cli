...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "turing.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "turing.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "turing.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "turing.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["turing.json", "turing.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "turing.json"),
        path.join(tmp.path, "turing.jsonc"),
        path.join(project, "turing.json"),
        path.join(project, "turing.jsonc"),
      ])
    })
  })
