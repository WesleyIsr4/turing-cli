...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.TURING_TEST ?? "")'), {
      env: {
        TURING_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "TURING_TEST_SHELL"], {
      shell: true,
      env: {
        TURING_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("TURING_TEST_SHELL=ok")
  })
