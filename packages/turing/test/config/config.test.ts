...
import { EffectFlock } from "@turing-ai/shared/util/effect-flock"
...
const managedConfigDir = process.env.TURING_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "turing.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "turing.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://turing.ai/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in turing config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://turing.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "turing.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["TURING_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { turing: { options: { apiKey: "{env:TURING_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["turing"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["TURING_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["TURING_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "turing.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .turing directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".turing")
...
test("loads agents from .turing/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".turing")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .turing/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".turing")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .turing/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".turing")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only TURING_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.TURING_CONFIG_DIR
  process.env.TURING_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.TURING_CONFIG_DIR
    else process.env.TURING_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable TURING_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "turing.json"),
        JSON.stringify({ $schema: "https://turing.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .turing config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".turing")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .turing config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "turing.json"),
        JSON.stringify({
          $schema: "https://turing.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .turing config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "turing.json"),
        JSON.stringify({
          $schema: "https://turing.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".turing")
...
// Note: preload.ts sets TURING_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .turing config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .turing directory config enables it
      const opencodeDir = path.join(dir, ".turing")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "turing.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/turing")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/turing")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "turing.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.turing/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.turing/plugin/demo.ts", "file:///project/.turing/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.turing/plugin/demo.ts"])
  })
...
describe("TURING_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["TURING_DISABLE_PROJECT_CONFIG"]
    process.env["TURING_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["TURING_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["TURING_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .turing/ directories when flag is set", async () => {
    const originalEnv = process.env["TURING_DISABLE_PROJECT_CONFIG"]
    process.env["TURING_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .turing directory with a command
          const opencodeDir = path.join(dir, ".turing", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .turing should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["TURING_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["TURING_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["TURING_CONFIG_DIR"]
      process.env["TURING_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["TURING_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["TURING_CONFIG_DIR"]
      } else {
        process.env["TURING_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("TURING_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["TURING_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["TURING_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["TURING_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["TURING_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from TURING_CONFIG_DIR, not project
...
describe("TURING_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in TURING_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["TURING_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["TURING_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://turing.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["TURING_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["TURING_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in TURING_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["TURING_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.turing.managed.test",
          PayloadType: "ai.turing.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info.zod,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://turing.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://turing.ai/config.json")
})
